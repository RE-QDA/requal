...
mod_user_manager_server <- function(id, glob) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$save_permissions, {
      ...
      dplyr::mutate(dplyr::across(tidyselect::matches("modify|view"), .fns = function(x) {
        x  <- 0 # reset permissions
      })) %>%
      dplyr::mutate(dplyr::across(tidyselect::all_of(input$permissions_list), .fns = function(x) {
