  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(glob$active_project, {
      if (golem::get_golem_options("mode") == "server") {
        ...
        loc$all_users <- get_users(
          credentials_path = golem::get_golem_options(
            which = "credentials_path"
          ),
          credentials_pass = golem::get_golem_options(
            which = "credentials_pass"
          )
        )

        loc$all_users_choices <- stats::setNames(
          loc$all_users$user_id,
          loc$all_users$user_login
        )
    ...
    observeEvent(input$save_permissions, {
      ...
      modified_permissions_df <- loc$users_permissions_df %>%
        dplyr::filter(!is.na(user_login)) %>%
        dplyr::filter(user_id %in% input$members_permissions) %>%
        dplyr::mutate(dplyr::across(
          dplyr::matches("modify|view"),
          .fns = function(x) {
            x <- 0 # reset permissions
          }
        )) %>%
        dplyr::mutate(dplyr::across(
          dplyr::all_of(input$permissions_list),
          .fns = function(x) {
      ...
      modified_permissions_df$permissions_modify[
        modified_permissions_df$project_owner == 1
      ] <- 1
    ...
    observeEvent(input$assign, {
      ...
      existing_users_check <- input$rql_users[
        !input$rql_users %in% existing_users
      ]
    ...
    observeEvent(input$remove_members, {
      ...
      if (any(owner_check == 1)) {
        warn_user("Project owners cannot be removed from project.")
      }
      ...
      # refresh users for current project
      loc$project_members_ids <- loc$project_members_ids[
        loc$project_members_ids != as.integer(input$members_to_remove)
      ]
    ...
    observeEvent(c(loc$users_permissions_df, glob$active_project), {
      loc$users_to_add <- loc$all_users_choices[
        !loc$all_users_choices %in% as.integer(loc$users_permissions_df$user_id)
      ]
    ...
    mod_rql_button_server(
      ...
      custom_tagList = tagList(
        ...
        rql_button_UI(ns("assign"), label = "Add users")
      ),
      glob,
    ...
    mod_rql_button_server(
      ...
      custom_tagList = tagList(
        rql_picker_UI(
          ns("members_to_remove"),
          "Select users:",
          none = "Users to remove"
        ),
