  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(req(loc$codes_menu_observer), {
      ...
      loc$code_tree_df <- data.frame(
        id = loc$codebook$code_id,
        name = loc$codebook$code_name,
        parent = loc$codebook$code_parent_id,
        is_draggable = FALSE,
        custom_tags = purrr::pmap_chr(
          list(
            code_id = loc$codebook$code_id,
            code_name = loc$codebook$code_name,
            code_desc = loc$codebook$code_description,
            code_color = loc$codebook$code_color
          ),
          function(
            code_id,
            code_name,
            ...
            as.character(generate_coding_tools_tree(
              ns = ns,
              code_id,
              code_name,
              code_color,
    ...
    observeEvent(loc$codebook, {
      mod_tree_server("doc_codes_ui", loc$code_tree_df)
    })
