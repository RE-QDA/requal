...
mod_document_code_server <- function(id, glob) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(req(loc$codes_menu_observer), {
      ...
      #     code_desc = ..4
      #   ))
      code_tree <- build_tree_structure(loc$codebook, "code_id", "code_parent_id")
        # Assuming `tree_structure` is the tree generated by `build_tree_structure`
  code_labels <- purrr::map(code_tree, ~generate_coding_tools_tree(
      ns = ns,
      code_id = .x$code_id,
      code_name = .x$code_name,
      code_color = .x$code_color,
      code_desc = .x$code_description,
      children = .x$children
    )
  )
  ...
  observeEvent(glob$analyze_link, {
    if (glob$doc_selector != glob$analyze_link$doc_id) {
      ...
      updateSelectInput(session = session, "doc_selector", choices = c("", glob$documents), selected = glob$doc_selector)
      #loc$codes_menu_observer  <- loc$codes_menu_observer + 1
      loc$text_observer <- loc$text_observer + 1
    }
  })
