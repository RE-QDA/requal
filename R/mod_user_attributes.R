...
mod_user_attributes_ui <- function(id) {
  ...
  tagList(
    fluidRow(
      class = "module_tools",
      mod_rql_button_ui(
        ns("attribute_create_ui"),
        label = "Create attribute",
        icon = "plus"
      ),
      mod_rql_button_ui(
        ns("attribute_delete_ui"),
    ...
    fluidRow(
      ...
      actionButton(
        ...
        icon = icon("chart-pie", verify_fa = FALSE)
      ),
...
mod_user_attributes_server <- function(id, glob) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(glob$active_project, {
      loc$user_attributes <- get_user_attributes_data_table(
        ns = ns,
        glob$pool,
    ...
    observeEvent(input$show_graph, {
      ...
      output$user_attributes_chart <- renderPlot({
        user_attributes_summary <- get_user_attributes_summary(
          glob$pool,
          glob$active_project
        )
        ...
        if (n_attributes > 0) {
          ...
          ggplot2::ggplot(
            user_attributes_summary,
            ggplot2::aes(x = "", y = share, fill = attribute_value)
          ) +
            ggplot2::geom_bar(stat = "identity") +
            ggplot2::geom_text(
              ggplot2::aes(label = attribute_value),
              position = ggplot2::position_stack(vjust = 0.5)
            ) +
            ggplot2::coord_polar(theta = "y", start = 0) +
            ggplot2::facet_wrap(
              ggplot2::vars(attribute_name),
              nrow = rows,
              ncol = cols
            ) +
            ggplot2::theme_void() +
    ...
    observeEvent(input$add_attribute, {
      ...
      if (!isTruthy(input$attribute_name) | !isTruthy(input$attribute_values)) {
        ...
        length(split_values(input$attribute_values)) !=
          length(unique(split_values(input$attribute_values)))
      ) {
      ...
      if (glob$user$data$attributes_other_modify == 1) {
        showModal(
          modalDialog(
            ...
            HTML(paste0(
              "Are you sure that you want to delete user attribute: <b>",
              user_attribute_name,
              "</b>?"
            )),
            footer = tagList(
        ...
        showModal(
          modalDialog(
            title = "Delete user attribute",
            paste0(
              "You don't have necessary permissions for deleting user attributes."
            ),
            footer = tagList(
              modalButton("Cancel")
            ),
