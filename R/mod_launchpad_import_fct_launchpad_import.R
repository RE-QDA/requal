...
.refi_get_selections <- function(xml_file, refi_ns, codebook) {
  ...
  txt_sel_desc <- purrr::map(
    sources,
    .refi_get_child,
    "//qda:PlainTextSelection/qda:Description"
  ) |>
    purrr::map(xml2::xml_text)
  txt_sel_coding <- purrr::map(sources, .refi_get_child, "/qda:Coding") |>
    purrr::map(xml2::xml_attrs) |>
    purrr::map(dplyr::bind_rows)
  txt_sel_coding_ref <- purrr::map(
    sources,
    .refi_get_child,
    "//qda:PlainTextSelection/qda:Coding/qda:CodeRef"
  ) |>
    purrr::map(xml2::xml_attrs) |>
    purrr::map(dplyr::bind_rows) |>
    purrr::map(~ dplyr::rename(.x, guid = targetGUID))
  # i need to match coding refs to codebook
  txt_sel_matched <- txt_sel_coding_ref |>
    purrr::map(~ dplyr::right_join(.x, imp_project$codebook))
  ...
  txt_sel <- purrr::map_df(sources, function(source) {
    text_source <- xml2::xml_find_first(
      source,
      "./qda:Description",
      ns = refi_ns
    ) |>
      xml2::xml_text()
    selections <- xml2::xml_find_all(
      source,
