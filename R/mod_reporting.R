...
mod_reporting_server <- function(id, glob) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(
      {
        req(input$reporting_tabset == "logs") | input$logs_refresh
      },
      {
            ...
            loc$logs_df <- loc$logs_df %>%
              dplyr::mutate(
                detail = purrr::map_chr(
                  .data$detail,
                  possibly_parse_payload_json
                )
              )
          } else {
            loc$logs_df <- loc$logs_df %>%
              dplyr::mutate(
                detail = purrr::map_chr(.data$detail, function(x) {
                  possibly_parse_payload_json(x, sanitize = TRUE)
                })
              )
          }
        } else {
    ...
    output$report_logs <- DT::renderDT(server = FALSE, {
      DT::datatable(
        req(loc$logs_df),
        filter = "top",
