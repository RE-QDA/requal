    ...
    observeEvent(req(golem::get_golem_options(which = "mode") == "local"), {
      ...
      observeEvent(input$project_create, {
        ...
        loc$db_path <- paste0(
          ...
          paste0(
            gsub(
              "[^a-zA-Z0-9]+",
              "",
              iconv(input$project_name, to = "ASCII//TRANSLIT")
            ),
            ".requal"
          )
        )
        ...
        if (file.exists(loc$db_path)) {
          warn_user(
            "Project with the same name already exists in the selected directory. Choose a different name."
          )
        }
    ...
    observeEvent(
      ...
      ),
      {
        observeEvent(input$project_create, {
          ...
          if (
            isTruthy(glob$user$project_admin) &&
              !(glob$user$user_id %in% existing_user_id)
          ) {
            ...
            warn_user(
              "Only users with project administration privileges can create new projects."
            )
