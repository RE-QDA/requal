...
test_that("{shinytest2} test", {
    ...
    # Open the memo for editing by clicking its title link in the table
    app$run_js("document.querySelector('#memo_ui_1-memo .memo_name').click()")
    app$wait_for_idle()

    # Change the memo text and save
    app$set_inputs(`memo_ui_1-memo_main_editor-memo_text_editor` = "Memo nevim")
    app$click("memo_ui_1-memo_main_editor-save")
    app$wait_for_idle()

    # Re-open the memo and confirm the edited text persisted
    app$run_js("document.querySelector('#memo_ui_1-memo .memo_name').click()")
    app$wait_for_idle()

    vals <- app$get_values()
    expect_equal(vals$input$`memo_ui_1-memo_main_editor-memo_text_editor`, "Memo nevim")
})
